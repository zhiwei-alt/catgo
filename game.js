...
this.ai.recordResult(!isPlayerWin, scoreDiff);
    ...
    // 数子法：单位是「子」，diff 是子数差；diffMoku 是近似目数差
    const deadInfo = (s.deadBlack || s.deadWhite)
      ? `，死子：黑${s.deadBlack}子/白${s.deadWhite}子`
      : '';
    const desc = `${winnerName}胜 ${s.diffMoku} 目（黑 ${s.black.toFixed(1)}子 : 白 ${s.white.toFixed(1)}子，贴${this.engine.komi}子${deadInfo}）`;
